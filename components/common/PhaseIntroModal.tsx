  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-500">
      <div className="bg-white rounded-[2rem] max-w-sm w-full p-8 shadow-2xl relative overflow-hidden border-4 border-white">
        <div className="absolute top-0 left-0 w-full h-32 bg-gradient-to-b from-ferty-beige to-white -z-10"></div>
        <button
          onClick={() => onClose(dontShow)}
          className="absolute top-4 right-4 p-2 bg-white rounded-full shadow-sm hover:scale-110 transition-transform"
        >
          <X size={20} className="text-ferty-coral" />
        </button>
        ...
        <h2 className="text-2xl font-bold text-center text-ferty-dark mb-2">{info.title}</h2>
        <p className="text-center text-ferty-gray text-sm mb-8 leading-relaxed">{info.description}</p>

        <div className="bg-ferty-beige/50 rounded-xl p-5 mb-6 border border-ferty-beige">
          <h4 className="text-[10px] uppercase font-bold text-ferty-coral tracking-widest mb-3">Objetivos Clave</h4>
          <ul className="space-y-3">
            {info.tasks.map((task, i) => (
              <li key={task} className="flex items-center gap-3 text-sm font-bold text-ferty-dark">
                <div className="w-5 h-5 rounded-full bg-ferty-rose text-white flex items-center justify-center text-[10px] shadow-sm">{i + 1}</div>
                {task}
              </li>
            ))}
          </ul>
        ...
        <div className="flex items-center gap-2 mb-6 justify-center">
          <input
            ...
            className="w-4 h-4 rounded border-stone-300 text-ferty-rose focus:ring-ferty-rose"
          />
          <label htmlFor="dontShow" className="text-xs text-ferty-gray">No volver a mostrar esta pantalla</label>
        </div>

        <button
          onClick={() => onClose(dontShow)}
          className="w-full bg-ferty-dark text-white py-4 rounded-xl font-bold shadow-xl hover:scale-[1.02] transition-transform"
        >
          ¡Vamos allá!
        </button>
      </div>
