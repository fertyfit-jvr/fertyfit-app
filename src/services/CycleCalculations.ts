...
// ============================================================================
// 8. CÁLCULO DE FECHA DE INICIO DE CICLO ACTUAL
// ============================================================================

/**
 * Calcula la fecha de inicio del ciclo actual proyectada
 * Si la última regla fue hace mucho tiempo, proyecta cuándo debió empezar el ciclo actual
 * asumiendo regularidad.
 */
export function calcularInicioCicloActual(ultimaRegla: string | Date, duracionCiclo: number): Date {
    const fechaUltima = new Date(ultimaRegla);
    const hoy = new Date();

    // Si la fecha es futura o inválida, devolvemos la original
    if (fechaUltima > hoy || isNaN(fechaUltima.getTime())) {
        return fechaUltima;
    }

    // Calcular días transcurridos
    const diffTime = hoy.getTime() - fechaUltima.getTime();
    const diasTranscurridos = Math.floor(diffTime / (1000 * 60 * 60 * 24));

    // Si no ha pasado ni un ciclo, la fecha es la misma
    if (diasTranscurridos < duracionCiclo) {
        return fechaUltima;
    }

    // Calcular cuántos ciclos completos han pasado
    const ciclosCompletados = Math.floor(diasTranscurridos / duracionCiclo);

    // Proyectar nueva fecha
    const nuevaFecha = new Date(fechaUltima);
    nuevaFecha.setDate(nuevaFecha.getDate() + (ciclosCompletados * duracionCiclo));
